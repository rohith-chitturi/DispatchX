import React, { useState } from 'react';
import { useAuth } from '../contexts/AuthContext';

interface ReceiptModalProps {
  isOpen: boolean;
  onClose: () => void;
  fare: string | null;
  rideId: string | null;
}

export const ReceiptModal: React.FC<ReceiptModalProps> = ({ isOpen, onClose, fare, rideId }) => {
  const { token } = useAuth();
  const [ratingMode, setRatingMode] = useState(false);
  const [rating, setRating] = useState(0);
  const [hoverRating, setHoverRating] = useState(0);

  if (!isOpen || !fare) return null;

  const submitRating = async () => {
    if (!rideId || rating === 0) return;
    try {
      await fetch('http://localhost:3000/api/rides/rate', {
        method: 'POST',
        headers: { 
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}` 
        },
        body: JSON.stringify({ rideId, rating })
      });
      // Close everything regardless of error for MVP
      setRatingMode(false);
      setRating(0);
      onClose();
    } catch (err) {
      console.error(err);
      onClose();
    }
  };

  const handleInitialClose = () => {
    setRatingMode(true);
  };

  return (
    <div className="fixed inset-0 z-[2000] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fade-in">
      <div className="bg-zinc-900 border-2 border-dispatch-neon/50 rounded-3xl p-8 w-full max-w-sm shadow-[0_0_50px_rgba(0,255,255,0.15)] relative">
        {ratingMode ? (
          <div className="flex flex-col items-center text-center">
            <h2 className="text-2xl font-black tracking-tight text-white mb-2">
              RATE YOUR <span className="text-dispatch-neon">DRIVER</span>
            </h2>
            <p className="text-zinc-400 font-mono text-sm mb-6">
              How was your trip?
            </p>
            
            <div className="flex gap-2 mb-8">
              {[1, 2, 3, 4, 5].map((star) => (
                <button
                  key={star}
                  onMouseEnter={() => setHoverRating(star)}
                  onMouseLeave={() => setHoverRating(0)}
                  onClick={() => setRating(star)}
                  className={`text-5xl transition-transform transform hover:scale-110 ${
                    star <= (hoverRating || rating) ? 'text-yellow-400 drop-shadow-[0_0_10px_rgba(250,204,21,0.5)]' : 'text-zinc-700'
                  }`}
                >
                  ★
                </button>
              ))}
            </div>

            <button 
              onClick={submitRating}
              disabled={rating === 0}
              className={`w-full font-black py-4 rounded-xl transition-all text-lg ${
                rating > 0 
                  ? 'bg-dispatch-neon text-black hover:bg-cyan-300' 
                  : 'bg-zinc-800 text-zinc-500 cursor-not-allowed'
              }`}
            >
              SUBMIT RATING
            </button>
            <button 
              onClick={() => {
                setRatingMode(false);
                onClose();
              }}
              className="mt-4 text-sm font-mono text-zinc-500 hover:text-white transition-colors"
            >
              SKIP
            </button>
          </div>
        ) : (
          <div className="flex flex-col items-center text-center">
            <div className="w-16 h-16 bg-dispatch-success/20 rounded-full flex items-center justify-center mb-4 border border-dispatch-success/50">
              <span className="text-3xl">✅</span>
            </div>
            
            <h2 className="text-2xl font-black tracking-tight text-white mb-2">
              RIDE <span className="text-dispatch-success">COMPLETED</span>
            </h2>
            
            <p className="text-zinc-400 font-mono text-sm mb-8">
              Your driver has dropped you off.
            </p>
            
            <div className="bg-black/50 border border-zinc-800 w-full rounded-2xl p-6 mb-8">
              <div className="text-sm font-mono text-zinc-500 mb-2">TOTAL FARE</div>
              <div className="text-5xl font-black text-white">
                <span className="text-dispatch-neon mr-1">$</span>{fare}
              </div>
            </div>
            
            <button 
              onClick={handleInitialClose}
              className="w-full bg-white hover:bg-zinc-200 text-black font-black py-4 rounded-xl transition-transform transform active:scale-95 text-lg"
            >
              PAY & CLOSE
            </button>
          </div>
        )}
      </div>
    </div>
  );
};
